...
public class KThread {
	...
	public static void finish() {
		...
		if(currentThread.joinQueue.isEmpty() == false){
			currentThread.joinQueue.remove(0).ready();
		}
	...
	*/
	...
	private static class MyTester implements Runnable {
	...
	public static void selfTest() {
		...
		KThread simpleWorker = new KThread(new Runnable() { // Join
			public void run() {
				for (int i = 0; i < 100; i++)
					KThread.yield();
					workerDone = true;
		...
		Communicator c = new Communicator(); // Communicator
		...
		simpleWorker.fork(); // Fork off the worker
		simpleWorker.join(); // Join on the worker
		System.out.println("Worker is done! " + (workerDone == true)); // Assert that the worker completed before join() returned
		...
		System.out.println("Testing commmunicator class.");
		thread1.fork();
		thread2.fork();

		thread1.join();
		thread2.join();
		System.out.println("Correct Message recieved -- " + (0xdeadbeef == l.getMessage()));
	...
	private static class Listener implements Runnable {
		...
		private int getMessage() {
			System.out.println("Listener has finished running --" +hasRun);
			return msg;
		}
	}
	...
	*/
	...
	private List<KThread> joinQueue = null;	
